...
class SimInfo:
    ...
    def __init__(self, snap, frames, ixn_file = "interactions.txt", cutoff_mult = 1.35, radius_mult = 1.2):
        ...
        #determine the number of subunits and nanoparticles from the snap
        self.num_particles = 0
        self.num_nanos     = 0
        self.__get_num_particles(snap)

        #check if the number of particles is zero and throw error
        if (self.num_particles == 0):
            raise ValueError("No particles could be found based on the type naming"\
    ...
    def __parse_interactions(self, ixn_file):
        ...
        with open(ixn_file, 'r') as f:
            for line in f:
                ...
                #check the number of entries. If 2, this is a bond. If 1, nanoparticle. 
                if len(entries) == 3:
                    self.bonds.append(entries)
                elif len(entries) == 2:
                    self.nanos.append(entries)
                    self.nano_flag = True
                else:
        ...
        bond_lengths = [bond[2] for bond in self.bonds]
        self.largest_bond_distance = np.max(bond_lengths) * 0.75

        return


    def __get_num_particles(self, snap):
        ...
        #loop over atom types in the bonds array and add them to an array
        # TODO - watch out for double counting bonds
        atom_types = []
        for pair in self.bonds:
            atom_types.append(pair[0])
            atom_types.append(pair[1])
        ...
        self.num_particles = len(body_set)

        #loop over nanoparticle entries to add types to array
        nano_types = []
        for entry in self.nanos:
            nano_types.append(entry[0])
        ...
        for nano_type in nano_types:
            ...
            nano_list = particle_info.loc[condition]['body']
