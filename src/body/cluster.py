...
class Frame:
    ...
    def get_similarity(self, old_cluster, new_cluster):
    ...
    def update(self, cluster_info, old_frame, observer):
        #use the previous frame data to update the new frame
        ...
        for key in list(self.__label_dict.keys()):
            ...
            cluster_info[key].update_data(updated_cluster, frame_num, observer.current_monomer)
        ...
        for update_pair in merge_updates:
            ...
            cluster_info[old_id].update_data(current_cluster, frame_num, observer.current_monomer)
    ...
    def __determine_event(self, possibleMatches):
        #return what kind of update to do based on length of possible matches
...
def get_data_from_snap(snap, sim, frame):
    ...
    current_frame = Frame(bodies, clusters, frame, monomer_frac) 
    return current_frame



def track_clustering(snap, sim, frame, cluster_info, old_frame, observer):
    #compute and update cluster stats from the given frame

    #determine clusters and init a Frame object with the needed data
    current_frame = get_data_from_snap(snap, sim, frame)
